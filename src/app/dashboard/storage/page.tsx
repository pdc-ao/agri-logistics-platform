...
export default async function StoragePage() {
  ...
  // Example: fetch storage facilities or items
      ...
      {facilities.length === 0 ? (
        ...
        <ul className="space-y-4">
          {facilities.map((facility) => (
            ...
            >
              <div>
                <h2 className="font-medium">{facility.name}</h2>
                <p className="text-xs text-neutral-500">
                  Manager: {facility.manager?.username || "Unassigned"}
                </p>
              </div>
              <span className="text-sm text-neutral-600">
                Capacity: {facility.capacity} {facility.unit}
              </span>
            </li>
          ))}
        </ul>
